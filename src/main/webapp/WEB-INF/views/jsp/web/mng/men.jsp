<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<style type="text/css">

</style>

<div class="model_kategorie_box">
	<table class="model_kategorie">
		<tr>
			<th><a href="">Women</a></th>
			<th><a href="">Men</a></th>
			<th><a href="">Development</a></th>
			<th><a href="">Actors</a></th>
		</tr>
	</table>
</div>
<div class="model_search_bar">
	<form action="/" method="get" class="search_model" name="model_search_form">
		<input type="text" id="search" placeholder="search">
		<input type="button" value="검색" >
	</form>
</div>
<div>
	<table class="model_profile">
		<tr>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
		</tr>
		<tr>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
		</tr>
		<tr>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
			<td class="profile_list"><a href=""><img src="http://placehold.it/218x218"></a></td>
		</tr>
	</table>
</div>
<div class="model_page_box">
	<div class="model_page_list">
		<a class="model_page_number" href="">1</a>
		<a class="model_page_number" href="">2</a>
		<a class="model_page_number" href="">3</a>
		<a class="model_page_number" href="">4</a>
		<a class="model_page_number" href="">5</a>
		<a class="model_page_number" href="">6</a>
		<a class="model_page_number" href="">7</a>
		<a class="model_next_page" href="">다음</a>
	</div>
</div>
